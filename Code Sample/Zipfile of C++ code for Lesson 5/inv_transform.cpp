...
float get_uniform()
{
	...
	// use the above routine if your compiler cannot find random()
	// random() returns an unsigned integer between 1 and (2^31-1)
	// it is better than rand()
...
int main (int argc, char* argv[])
{
	...
	for (int j = 0; j < 100; j++)
		outf << ((float) j/10) << ", " << 
		((float) count[j])/((float) no_of_trials) << ", " << 
		(1.0-exp(-1.0*lambda*((float) j/10))) << endl;
